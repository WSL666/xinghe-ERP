...
from core.base import PipelineStepError, MULTIMODAL_MAX_ATTEMPTS, MULTIMODAL_TIMEOUT, log
from llm.base import ApiKeyError
from llm.factory import get_multimodal_client
from api_key_pool import get_pool


def build_multimodal_messages(prompt: str, image_b64_list: list[str]) -> list[dict[str, Any]]:
...
def analyze_product(env: dict[str, str], prompt: str,
    ...
    """Call the Multimodal model to analyze all images -> output JSON."""
    client = get_multimodal_client(env, api_key=api_key)
    return client.analyze(prompt, image_b64_list, timeout=MULTIMODAL_TIMEOUT)


def validate_analysis_payload(payload: dict[str, Any],
                               image_count: int) -> tuple[list[int], list[tuple[int, str]]]:
    """Validate Multimodal result."""
...
def analyze_product_with_retry(
    ...
    valid_b64: list[str],
    image_count: int,
    max_attempts: int = MULTIMODAL_MAX_ATTEMPTS,
) -> dict[str, Any]:
    """Multimodal analysis with retry + key 池轮换。"""
    pool = get_pool("chat")
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            payload = analyze_product(env, multimodal_prompt, valid_b64, api_key=api_key)
            ...
            attempts.append(attempt_info)
            log(f"[WARN] Multimodal attempt {attempt}/{max_attempts} failed: {exc}")
    ...
    raise PipelineStepError(f"Multimodal analysis failed after {max_attempts} retries: {last_error}", {
        "attempts": attempts,
