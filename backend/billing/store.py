...
def hold_amount_for(features: list[str]) -> int:
    ...
    - ["llm"]            → 1 (LLM文本)
    - ["image_gen"]       → 10 (生图: 多模态1 + 生图上限, 固定10)
    - ["llm","image_gen"]   → 11 (全链路)
    """
    amt = 0
    if "llm" in features:
        amt += COST_TITLE
    if "image_gen" in features:
        amt += HOLD_IMAGES
    return amt
...
def reset_billing_for_import(user_id: int, import_id: int) -> None:
    ...
    操作(原子, FOR UPDATE 锁行):
      1. 查该 import 的旧 hold 额度 → 从 frozen 里减掉(把冻结还回去)
      2. 查实际扣费(amount < 0)总额 → 退回到 beans
      3. 删除该 import 的所有 bean_transactions 记录
    注意: 不动其他 import 的 frozen, 保持全局 frozen 余额一致。
    """
    with db_conn() as conn:
        ...
        # 查该 import 的旧 hold 额度(从 ai_features 算)
        ...
        if held_rows:
            ...
            # 退还 frozen(把这次 import 的冻结额度还回可用余额)
