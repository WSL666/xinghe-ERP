...
let cachedPipelineConfig = {};

// Pipeline backend address. Distributed to internal staff over the LAN, so this
// must point at the server's LAN IP (not localhost, which would mean each user's
// own machine). Change here if the server IP changes.
const DEFAULT_PIPELINE_URL = 'http://192.168.0.104:6688';
const SHOP_FIELDS = [
...
function normalizePipelineUrl(value) {
  return String(value || DEFAULT_PIPELINE_URL).trim().replace(/\/+$/, '');
}

function updatePipelineStatus() {
  chrome.storage.local.get(['pipelineConfig'], result => {
    const cfg = result.pipelineConfig || {};
    cachedPipelineConfig = cfg;
    const urlInput = document.getElementById('pipeline-url');
    const keyInput = document.getElementById('pipeline-api-key');
    const statusEl = document.getElementById('pipeline-status');
    if (urlInput) urlInput.value = cfg.url || DEFAULT_PIPELINE_URL;
    if (keyInput) keyInput.value = cfg.apiKey || '';
    if (!statusEl) return;
    if (cfg.url && cfg.apiKey) {
      statusEl.textContent = `Configured: ${cfg.url}`;
...
async function savePipelineConfig() {
  const url = normalizePipelineUrl(document.getElementById('pipeline-url')?.value);
  const apiKey = String(document.getElementById('pipeline-api-key')?.value || '').trim();
  await chrome.storage.local.set({ pipelineConfig: { url, apiKey } });
  cachedPipelineConfig = { url, apiKey };
  updatePipelineStatus();
}

async function getPipelineConfig() {
  const result = await chrome.storage.local.get(['pipelineConfig']);
  const cfg = result.pipelineConfig || {};
  return {
    url: normalizePipelineUrl(cfg.url),
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('pipeline-save-btn')?.addEventListener('click', e => {
    e.stopPropagation();
    savePipelineConfig();
  });
});
...
document.getElementById('exportBtn').addEventListener('click', async () => {
  ...
  try {
    ...
    if (dbResp.ok) {
      ...
      // ----- schema 2：refPid 主键查 pid；templates 按 templateId 精确取 templatePid；props 兼容层兜底 -----
      ...
      const pageTemplateId = String(collectedData.templateId || '');  // 若以后采集到类目模板则精确
      let hitCount = 0;
      // 过滤不需要的属性
      ...
      enrichedProps = filteredProps.map(p => {
        ...
        // 1) pid：refPid 主键优先（1:1 最干净），其次名字，最后兼容层
        let pid = (ref && refPidDB[ref] != null) ? refPidDB[ref]
                : (namesDB[pn] != null ? namesDB[pn]
                : (propsDB[pn] ? propsDB[pn].pid : ''));
        if (pid === '' || pid == null) return p; // 数据库中没有该属性，保持原样
        hitCount++;
        // 2) templatePid：知道类目模板就精确，否则默认兜底，再不行用兼容层/原值
        let tpid = '';
        if (pageTemplateId && templatesDB[pageTemplateId] && templatesDB[pageTemplateId][pid] != null)
          tpid = templatesDB[pageTemplateId][pid];
        else if (defTpidDB[pid] != null) tpid = defTpidDB[pid];
        else if (propsDB[pn] && propsDB[pn].templatePid) tpid = propsDB[pn].templatePid;
...
document.getElementById('sendPipelineBtn').addEventListener('click', async () => {
  ...
  try {
    ...
    if (!pipelineCfg.apiKey) {
      throw new Error('Missing API Key. Configure it in Pipeline Config first.');
    }
    ...
    const res = await fetch(pipelineCfg.url + '/api/plugin/import', {
      method: 'POST',
      headers: {
